...
% Pretty fonts
\usepackage{mathpazo} % Changes the default text font to the "Palatino" typeface

% Bibliography
% - https://www.utwente.nl/en/service-portal/university-library/referencing-copyright/referencing-plagiarism
\usepackage[authoryear]{natbib}
\bibliographystyle{apalike} % Other styles: plain, unsrt, abbrvnat, and apalike.
...
\usepackage{siunitx} % https://texdoc.org/serve/siunitx/0
...
% Import SVGs
% Note: some TeX distributions ship an older svg.sty that
% does not support the `outputdir` option; use defaults.
\usepackage[inkscapepath="figures-converted"]{svg} % If images do not work (e.g., missing pattern) convert to PNG using inkscapeformat=png

\usepackage{epstopdf}
%\graphicspath{{figures/}} % Tells LaTeX to look in the Images/ folder for figures
\epstopdfsetup{outdir=./figures-converted/} % Tells epstopdf to save converted PDFs here
% LISTS
